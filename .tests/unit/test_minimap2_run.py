...
def test_minimap2(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/minimap2/config")
        data_path = Path(".tests/unit/minimap2/data")
        expected_path = Path(".tests/unit/minimap2/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "results/minimap2/A1163_mapping.sam",
                "--snakefile",
                "workflow/Snakefile",
                "-f",
                "--notemp",
