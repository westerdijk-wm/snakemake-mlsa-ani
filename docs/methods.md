...
![Workflow DAG generated by `snakemake --report`, showing the rule graph for a default run (IQ-TREE phylogenetic inference, skani ANI analysis, and inclusion of public genomes from NCBI).](workflow_visualization.png)

*Figure: Rule graph of the snakemake-MLSA-ANI workflow, generated via `snakemake --report` using the default configuration (`tree.method: iqtree`, `ani_method: skani`) with public genomes enabled. Each node represents a rule; arrows indicate input/output dependencies between rules, from public genome download, initial genome QC, and reference gene validation through gene extraction, alignment, concatenation, phylogenetic inference, and ANI analysis.*
