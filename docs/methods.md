...
![Workflow DAG generated by `snakemake  --cores 30 --use-conda --report`, showing the rule graph for a default run (IQ-TREE phylogenetic inference, skani ANI analysis, and inclusion of public genomes from NCBI).](workflow_visualization.png)
...
Phylogenetic reconstruction is performed using one of three selectable methods, configured via `tree.method` (see [Configuration](configuration.md)).
...
The number of bootstrap replicates is user configurable (minimum 100, recommended ≥ 1000).
...
The number of bootstrap replicates is user configurable (minimum 1, recommended ≥ 100).
...
FastTree provides a computationally efficient alternative for larger datasets. **Note that FastTree is not partition aware and does not perform bootstrapping**; the `tree.bootstrap` setting is ignored when this method is selected.

### Tree rooting

The resulting phylogeny is rerooted using `nw_reroot` from the [Newick Utilities](https://github.com/tjunier/newick_utils) package and exported in Newick format as `phylogenetics/MLSA.nwk`.

## Average Nucleotide Identity (ANI)

ANI analysis is optional, configured via `ani_method` (see [Configuration](configuration.md)), and is performed only on genomes that pass all gene QC criteria.
